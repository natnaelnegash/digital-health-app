...
const AppointmentConfig = () => {
  ...
  const [provider, setProvider] = useState(""); // store provider ID
  ...
  const handleSave = (e) => {
    ...
    const config = {
      id: Date.now(),
      slotDuration,
      provider: selectedProvider, // save full provider object
  ...
  const handleLoadConfig = (config) => {
    ...
    setProvider(config.provider.id); // load provider ID
            ...
            <div className="space-y-2">
              {savedConfigs.map((config) => (
                <div key={config.id} className="flex justify-between items-center p-3 bg-white rounded-lg border">
                  <div>
                    <span className="font-medium">{config.provider.firstName} {config.provider.lastName}</span>
        ...
        <div>
          ...
          <div className="space-y-4">
            {availability.map((slot, index) => (
              <div key={index} className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 p-4 border rounded-xl border-gray-200">
                <select
                  value={slot.day}
